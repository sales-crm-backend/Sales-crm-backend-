...
def log_activity(user_id: str, action: str, entity_type: str, entity_id: str, details: dict = None):
    activity = {"user_id": user_id, "action": action, "entity_type": entity_type, "entity_id": entity_id, "details": details or {}, "timestamp": datetime.utcnow()}
    activity_log_collection.insert_one(activity)

def init_default_users():
    if users_collection.count_documents({}) == 0:
        admin_user = {"username": "admin", "password": get_password_hash("admin123"), "full_name": "Admin User", "role": "manager", "phone": None, "created_at": datetime.utcnow(), "is_active": True}
...
def login(user_login: UserLogin):
    ...
    access_token = create_access_token(data={"sub": str(user["_id"]), "username": user["username"], "role": user["role"]})
...
def register(user_create: UserCreate, current_user: dict = Depends(get_current_manager)):
    ...
    log_activity(current_user["user_id"], "created", "user", str(result.inserted_id), {"username": user_create.username})
...
def create_lead(lead: Lead, current_user: dict = Depends(get_current_user)):
    ...
    log_activity(current_user["user_id"], "created", "lead", str(result.inserted_id), {"lead_name": lead.name})
    return lead_dict
...
def get_leads(current_user: dict = Depends(get_current_user)):
    ...
    for lead in leads:
        ...
        if lead.get("assigned_to"):
            ...
            if assigned_user:
                lead["assigned_user"] = {"id": str(assigned_user["_id"]), "name": assigned_user.get("full_name", assigned_user["username"]), "username": assigned_user["username"]}
...
def assign_lead(assignment: LeadAssignment, current_user: dict = Depends(get_current_manager)):
    ...
    leads_collection.update_one({"_id": ObjectId(assignment.lead_id)}, {"$set": {"assigned_to": assignment.assigned_to, "assigned_date": datetime.utcnow().isoformat(), "updated_at": datetime.utcnow()}})
    log_activity(current_user["user_id"], "assigned", "lead", assignment.lead_id, {"assigned_to": assignment.assigned_to})
...
def create_followup(followup: FollowUp, current_user: dict = Depends(get_current_user)):
    ...
    leads_collection.update_one({"_id": ObjectId(followup.lead_id)}, {"$set": {"next_followup_date": followup.followup_date, "updated_at": datetime.utcnow()}})
    log_activity(current_user["user_id"], "created", "followup", str(result.inserted_id), {"lead_id": followup.lead_id})
...
def get_today_followups(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] in ["manager", "admin"]:
        followups = list(followups_collection.find({"followup_date": today, "status": "pending"}))
    else:
        lead_ids = [str(l["_id"]) for l in leads_collection.find({"assigned_to": current_user["user_id"]})]
        followups = list(followups_collection.find({"lead_id": {"$in": lead_ids}, "followup_date": today, "status": "pending"}))
    for followup in followups:
        followup = serialize_doc(followup)
        lead = leads_collection.find_one({"_id": ObjectId(followup["lead_id"])})
        if lead:
            followup["lead_info"] = {"name": lead.get("name"), "phone": lead.get("phone"), "lead_status": lead.get("lead_status"), "lead_stage": lead.get("lead_stage")}
    return [serialize_doc(f) for f in followups]
...
def get_overdue_followups(current_user: dict = Depends(get_current_user)):
    today = date.today().isoformat()
    if current_user["role"] in ["manager", "admin"]:
        followups = list(followups_collection.find({"followup_date": {"$lt": today}, "status": "pending"}))
    else:
        lead_ids = [str(l["_id"]) for l in leads_collection.find({"assigned_to": current_user["user_id"]})]
        followups = list(followups_collection.find({"lead_id": {"$in": lead_ids}, "followup_date": {"$lt": today}, "status": "pending"}))
    for followup in followups:
        followup = serialize_doc(followup)
        lead = leads_collection.find_one({"_id": ObjectId(followup["lead_id"])})
        if lead:
...
def create_comment(comment: Comment, current_user: dict = Depends(get_current_user)):
    ...
    log_activity(current_user["user_id"], "created", "comment", str(result.inserted_id), {"lead_id": comment.lead_id})
    return comment_dict
...
def create_order(order: Order, current_user: dict = Depends(get_current_user)):
    ...
    leads_collection.update_one({"_id": ObjectId(order.lead_id)}, {"$set": {"lead_status": "deal_closed", "updated_at": datetime.utcnow()}})
    log_activity(current_user["user_id"], "created", "order", str(result.inserted_id), {"lead_id": order.lead_id, "deal_amount": order.deal_amount})
    return order_dict
...
def get_manager_stats(current_user: dict = Depends(get_current_user)):
    ...
    today = date.today().isoformat()
    if current_user["role"] in ["manager", "admin"]:
        today_followups = followups_collection.count_documents({"followup_date": today, "status": "pending"})
        overdue_followups = followups_collection.count_documents({"followup_date": {"$lt": today}, "status": "pending"})
    else:
        lead_ids = [str(l["_id"]) for l in leads_collection.find({"assigned_to": current_user["user_id"]})]
        today_followups = followups_collection.count_documents({"lead_id": {"$in": lead_ids}, "followup_date": today, "status": "pending"})
        overdue_followups = followups_collection.count_documents({"lead_id": {"$in": lead_ids}, "followup_date": {"$lt": today}, "status": "pending"})
    active_sales_persons = users_collection.count_documents({"role": "sales_person", "is_active": True})
    first_day_of_month = datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    monthly_orders = list(orders_collection.find({"created_at": {"$gte": first_day_of_month}}))
    ...
    return {"total_leads": total_leads, "unassigned_leads": unassigned_leads, "statuses": statuses, "today_followups": today_followups, "overdue_followups": overdue_followups, "active_sales_persons": active_sales_persons, "monthly_sales": monthly_sales, "monthly_orders": monthly_order_count, "conversion_ratio": conversion_ratio, "closed_deals": closed_deals}
